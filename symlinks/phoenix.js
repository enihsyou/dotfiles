...
// ref: https://lukesmurray.com/blog/quake-applications-for-mac
...
Key.on("r", ["cmd", "ctrl"], async function () {
    Phoenix.log("Triggered")
    const space = Space.active();
    Phoenix.log(JSON.stringify({
        isFullScreen: space.isFullScreen(),
        isNormal: space.isNormal(),
        windows: space.windows().length,
        screens: space.screens().length,
    }))
    const app = App.launch("kitty", {focus: true});
...
function moveAppToActiveSpace(app, followsMouse) {
    ...
    if (activeSpace !== undefined) {
        ...
        if (moved) {
            ...
            // then add window to active space
