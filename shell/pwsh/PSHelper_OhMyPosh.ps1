# 这个文件是 Oh-My-Posh 26.24.0 的 PowerShell 版本的初始化脚本
...
# [enihsyou] 替换为运行时计算
# 替换成硬编码的结果跳过 PATH 解析，能节约 6ms
$global:_ompExecutable = "$Env:LOCALAPPDATA\Programs\oh-my-posh\bin\oh-my-posh.exe"
...
New-Module -Name "oh-my-posh-core" -ScriptBlock {
    ...
    # [enihsyou] 只要我不重复加载就可以不需要这部分，节约 72ms
    ...
    # [enihsyou] 替换为运行时计算
    $env:POSH_SESSION_ID = [guid]::NewGuid()
    $env:CONDA_PROMPT_MODIFIER = $false

    # [enihsyou] 从外部加载主题所在位置(变量自动绑定到调用点)
    # 最后选择直接禁用，由外部负责填充 $env:POSH_THEME，节约 2ms
    # set the default theme
    # if ("$POSH_THEME" -and (Test-Path -LiteralPath "$POSH_THEME")) {
    #     $env:POSH_THEME = (Resolve-Path -Path "$POSH_THEME").ProviderPath
    # }
    ...
    # [enihsyou] 见 $script:OriginalContinuationPrompt 部分注释
